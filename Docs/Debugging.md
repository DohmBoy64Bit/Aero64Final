...
- **Font:** `config/cmake/AeroEngine.cmake` prefers **`assets/DejaVuSans.ttf`** in the repo if it is non-trivial size; otherwise it downloads the **SourceForge** **`dejavu-fonts-ttf-2.37.tar.bz2`**, extracts **`ttf/DejaVuSans.ttf`**, and **`POST_BUILD`** copies it next to the executable as **`DejaVuSans.ttf`** (explicit destination path). The old single-file **GitHub raw** URL often produced **0-byte** files while still satisfying `EXISTS`, which broke the font until reconfigure. **`src/host/aero_engine_launcher.cpp`** probes several paths (SDL **`SDL_GetBasePath()`**, **`../aero_engine_assets/`**, cwd, Windows exe directory), skips files **≤ 64 KiB**, and falls back to **Segoe UI** via RML **`font-family`** if DejaVu never loads.
...
- **Boot tail before Gfx (`func_80231A24`):** After **`func_D_8020015C`**, **`static_3_80231A24`** (manual in **`RecompiledFuncs/aero_boot_chain_manual.c`**) runs the ROM **`jal`** sequence **`0x8023CE80` → `0x80237E10` → `0x8023F020` → `0x80230F68`**. **`src/host/aero_recomp_host.cpp`** calls **`recomp::overlays::add_loaded_function`** for **`aero_overlay_stub_*`** only when **`recomp::overlays::find_loaded_function`** returns **nullptr** (vendored **`lib/N64ModernRuntime/librecomp/src/overlays.cpp`**) so a future N64Recomp overlay entry is **not** overwritten by stubs. **`config/aero.us.toml`** lists **`function_sizes`** for those four names (spans from **`data_dump.toml`**). Workflow: WSL **`AERO_LINK_MODE=splatasm bash tools/scripts/build_aero_us_elf.sh`** (produces **`build/us/aero.us.splatasm.elf`**), then Windows **`tools\N64Recomp.exe config\aero.us.toml`** (if the CLI faults with **`-1073741819`**, rebuild N64Recomp, try **`aero.us.regen.toml`**, or build N64Recomp for Linux under WSL when **`cmake`** is installed). After regen, you can delete stub implementations in **`aero_boot_chain_manual.c`** when they are unused; **`static_3_80231A24`** stays manual until a correct full **`func_80231A24`** span exists (next symbol **`func_80231A40`** is only **+0x1C** — likely mid-routine).
...
4. **`after_entrypoint` boot chain** — N64Recomp lowers **`jr $ra`** to a C **`return`**, so MIPS fall-through after **`0x80200084`** never runs. **`recomp::GameEntry::after_entrypoint`** (**`lib/N64ModernRuntime/librecomp/include/librecomp/game.hpp`**) is invoked from **`wait_for_game_started`** (**`lib/N64ModernRuntime/librecomp/src/recomp.cpp`**) immediately after **`entrypoint`**. **`src/host/aero_recomp_host.cpp`** sets it to **`aero_boot_after_entrypoint`**, which mirrors ROM **`0x8020008C..0x8020009C`**, sets **`$ra`** to **`0x802000A0`** (MIPS **`jal`** return), calls **`func_8022970C`**, then **`func_802000A0`** (VMA **`0x802000A0..0x802000DB`**), then **`func_802000DC`**. Recompiled **`func_802000DC`** also ends in C **`return`**, so the host then calls **`func_D_802000F4`** (manual slice **`0x802000F4..0x80200158`**, including **`func_80231C9C`** through **`0x80231D04`**, verified with **`mips-linux-gnu-objdump`** on **`build/us/aero.us.splatasm.elf`**). Until N64Recomp emits these slices from **`config/aero.us.toml`** **`function_sizes`**, the hand translation lives in **`RecompiledFuncs/aero_boot_chain_manual.c`**. **`func_80228DE0`** there duplicates N64Recomp until regen — remove duplicate **`funcs.h`** decls when **`{ name = "func_80228DE0", size = 0x34 }`** (and the rest of the boot slices) are emitted successfully.
