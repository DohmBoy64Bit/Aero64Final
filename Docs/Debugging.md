...
- **Pipeline tracing (stderr):** set **`AERO_TRACE_SP=1`** to log (1) **`osSpTaskStartGo`** from **`lib/N64ModernRuntime/librecomp/src/sp.cpp`** (recomp path when the game queues an SP task — after `fetch_aero_engine_deps.ps1`, run **`git -C lib/N64ModernRuntime apply ../../tools/patches/aero_librecomp_sp_trace.patch`** from the Aero repo root if that file is missing), and (2) the **Gfx thread** path **`AeroRT64Context::send_dl`** in **`src/host/aero_rt64_render_context.cpp`** (first 256 tasks; complements the unconditional first 24 **`[Aero64][Gfx]`** lines). Set **`AERO_TRACE_RSP=1`** to log **`get_rsp_microcode`** selection in **`src/host/aero_recomp_callbacks.cpp`** (non-**`M_GFXTASK`** RSP work uses **`recomp::rsp::run_task`** on the SP worker thread; **`M_GFXTASK`** is **not** dispatched through that path — see **`lib/N64ModernRuntime/ultramodern/src/events.cpp`** **`gfx_thread_func`** vs **`task_thread_func`**).
