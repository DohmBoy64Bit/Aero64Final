...
## Current Status
- **Phase**: 1 / 7 (host tools landed; ROM/splat/ELF next)
- **Completed Steps**: N64Recomp + RSPRecomp in `tools/`; root **`aero.us.splat.yaml`** + splat split for USA ROM; **`config/aero.us.toml`** has ROM path + header-derived **`entrypoint = 0x80200050`**; **`config/asm_include/macro.inc`** for GNU as; Workflow updated for splat → mips → recomp
- **Next Step**: First `tools/N64Recomp.exe config/aero.us.toml` on **`build/us/aero.us.elf`** (bootstrap blob ELF); iterate splat/asm + `AERO_LINK_MODE=splatasm` when symbols match Ghidra
...
- Next: N64Recomp + splat/ELF for Aero USA; expand `CMakeLists.txt` when `lib/` submodules exist.
