...
## Current Status
- **Phase**: 3 / 7 (splat + ELF pipelines; engine `lib/` integration still open)
- **Completed Steps**: N64Recomp + RSPRecomp in `tools/`; root **`aero.us.splat.yaml`** + per-TU **`rom_*.s`** split; **`config/aero.us.toml`** (ROM path + **`entrypoint = 0x80200050`**); **`tools/scripts/build_aero_us_elf.sh`** with **bootstrap** and **splatasm** modes; **`AERO_SPLATASM_REFRESH`** / **`AERO_SPLATASM_PREP`** + **`splatasm_autoglabel_jal.py`** / **`splatasm_fix_jal_local_labels.py`** (Track B — **`Docs/Debugging.md`**); **`config/asm_include/macro.inc`**; Workflow/Debugging/README aligned with those flags.
- **Next Step**: Run both parallel priorities in **`Docs/Workflow.md`** (“Parallel priorities” under Phase 3): **Track A** — N64Recomp on bootstrap **`build/us/aero.us.elf`** until emit is clean; **Track B** — **`lib/`** + **`CMakeLists.txt`** toward a host window. Use **`AERO_SPLATASM_REFRESH=1`** when you need a section-accurate splat-linked ELF for analysis or downstream tooling.
