...
- **Next Step**: Run **`tools/N64Recomp.exe config/aero.us.toml`** on **`build/us/aero.us.elf`** produced with **`AERO_LINK_MODE=bootstrap`** when you need a guaranteed loadable ELF; use **`AERO_SPLATASM_REFRESH=1`** (or iterative **`AERO_LINK_MODE=splatasm`** + Ghidra) when you need a section-accurate splat-linked ELF for analysis or downstream tooling.
