...
- **Next Step**: **`Docs/Workflow.md`** Phase 3 — build **`build/us/aero.us.splatasm.elf`** for **`config/aero.us.toml`**; if the Windows N64Recomp CLI faults on that ELF, use **`config/aero.us.regen.toml`** + **`Docs/Debugging.md`**. **`CMakeLists.txt`** already links **`RecompiledFuncs`** (Banjo **`add_library`** pattern); add **`lib/`** runtime when ready.
